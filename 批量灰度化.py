...
parser.add_argument('--color_img_dir', type=str, default=r'C:\Users\WangChun\Desktop\ColortoGray\input',        #设置高分辨率图片路径参数
                    help='path to Color image dir')
parser.add_argument('--gray_img_dir', type=str, default=r'C:\Users\WangChun\Desktop\ColortoGray\result',       #设置低分辨率路径参数
...
gray_image_dir = args.gray_img_dir              #从参数列表中取出低分辨率图像路径
 
print(args.color_img_dir)                      #将热红外热图像路径打印出来
print(args.gray_img_dir)                      #将热红外灰度图像路径打印出来
 
 
#create LR image dirs
#在低分辨率图像路径中创建每个下采样倍率的文件夹
os.makedirs(gray_image_dir, exist_ok=True)       #创建保存灰度图的文件夹，exist_ok：只有在目录不存在时创建目录，目录已存在时不会抛出异常       
...
for filename in os.listdir(color_image_dir):             #遍历热图像文件夹中的每一个文件
    ...
    h, w = color_img.shape[:2]  # 获取图片的high和wide
 
    img_gray = np.zeros([h, w], color_img.dtype)  # 创建一张和当前图片大小一样的单通道图片
    
    for i in range(h):          #灰度化临时创建的图片
        for j in range(w):
            m = color_img[i, j]  # 取出当前high和wide中的BGR坐标
            img_gray[i, j] = int(m[0] * 0.11 + m[1] * 0.59 + m[2] * 0.3)  # 将BGR坐标转化为gray坐标并赋值给新图像
    

 
    cv2.imwrite(os.path.join(gray_image_dir, filename.split('.')[0]+ext), img_gray)  #将文件写入准备好的灰度图片文件夹中
    num = num+1
    print(num)
